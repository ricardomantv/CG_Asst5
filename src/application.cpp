...
void Application::render() {
  ...
  switch (mode) {
    ...
    case ANIMATE_MODE:
      ...
      if (action == Action::IK && rightDown) {
        if (scene->has_selection()) {
          if (scene->selected.object->getInfo()[0] == string("JOINT")) {
            DynamicScene::Joint *j = (DynamicScene::Joint*)scene->selected.object;
      ...
      if (show_coordinates && !timeline.isCurrentlyPlaying()) draw_coordinates();
      timeline.step();
...
void Application::keyboard_event( int key, int event, unsigned char mods )
{
   switch( mode )
   {
      case ANIMATE_MODE:
        ...
        {
          case GLFW_KEY_TAB:
            if (event == GLFW_PRESS && action == Action::Object) scene->elementTransform->cycleMode();
            break;
          case GLFW_KEY_L:
          ...
          case GLFW_KEY_SPACE:
            if (event == GLFW_PRESS && (action != Action::Raytrace_Video && action != Action::Rasterize_Video)) {
              ...
              setGhosted(!timeline.isCurrentlyPlaying() && action != Action::Wave);
            }
            break;
          case GLFW_KEY_LEFT:
            switch (mods) {
            ...
            if (event == GLFW_PRESS && !timeline.isCurrentlyPlaying()) {
              if (mods) {
                ...
                if (action == Action::CreateJoint) {
                  ...
                  if (remaining == false) timeline.unmarkTime(timeline.getCurrentFrame());
                }
              }
            }
            break;
...
void CMU462::Application::writeSkeleton(const char * filename, const DynamicScene::Scene * scene)
{
    ...
    case LEFT:
      ...
      else if (mode == ANIMATE_MODE)
      {
        ...
        if (timeline.mouse_over(mouseX, mouseY)) {
          ...
          DynamicScene::Joint* joint = dynamic_cast<DynamicScene::Joint*>(scene->hovered.object);
          if (joint != nullptr) {
            selectHovered();
            clickedJoint = joint;
          }
  ...
  if (mode == MODEL_MODE ) {
    ...
    if (action == Action::CreateJoint) {}
      ...
      if (scene->has_selection()) {
        ...
        } else if (scene->elementTransform->mode == DynamicScene::XFormWidget::Mode::Scale) {
          target->scales.setValue(t, target->scale);
        } else if (scene->elementTransform->mode == DynamicScene::XFormWidget::Mode::Rotate) {
      ...
      if (scene->has_selection()) {
        double t = timeline.getCurrentFrame();
        auto joint = dynamic_cast<DynamicScene::Joint*>(scene->selected.object);
        if (joint == nullptr)
          dragSelection(x, y, dx, dy, get_world_to_3DH());
        auto target = (scene->selected.object != scene->elementTransform) ?
          (scene->selected.object) : (scene->elementTransform->target.object);
