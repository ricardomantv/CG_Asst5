  ...
  namespace CMU462 {
    ...
    class HalfedgeElement
    {
      public:
         ...
         */
        virtual void translate( double dx, double dy, const Matrix4x4& modelViewProj ) = 0;
      ...
      protected:
        /**
         * Helper function for translating a point in space given a desired change in screen space.
         */
        void translatePoint( Vector3D& p, double dx, double dy, const Matrix4x4& modelViewProj );
    };
    ...
    class HalfedgeMesh {
      public:
        ...
        void _bevel_vtx_reposition_with_dist(Vector3D orig, vector<HalfedgeIter>& hs, double inset );
        void _bevel_fc_reposition_with_dist(vector<Vector3D>& orig, vector<HalfedgeIter>& hs, double shift, double inset );
    ...
    inline Edge* HalfedgeElement::getEdge() {
      return dynamic_cast<Edge*>(this);
    }
    inline Face* HalfedgeElement::getFace() {
